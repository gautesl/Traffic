...
func _fixed_process(delta):
	...
	if is_colliding():
		if get_collider().is_in_group("wall"):
			strength = 0
		elif get_collider().is_in_group("enemy"):
			var lives = get_collider().lives
			pierced += 1
			if get_collider().lives <= strength: source.xp += pierced
			get_collider().set_lives(get_collider().lives -strength, source)
			strength -= lives
			if bazooka:
				strength = 0
...
func collision(body):
	if body.is_in_group("wall") and not body.get_name() in exceptions:
		strength = 0
	elif body.is_in_group("enemy"):
		var lives = body.lives
		pierced += 1
		if body.lives <= strength: source.xp += pierced
		body.call_deferred("set_lives", body.lives -strength, source)
		strength -= lives
		if bazooka:
			strength = 0
			var directions = [Vector2(0,-1),Vector2(1,-1),Vector2(1,0),Vector2(1,1),
...
func rotate(bullet_strength, pos=Vector2(0, -110), time=60, circling=true, collision_exception=null):
	...
	timer.start()
	if collision_exception: exceptions.append(collision_exception.get_name())
